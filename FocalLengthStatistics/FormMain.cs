...
namespace FocalLengthStatistics
{
    public partial class FormMain : Form
    {
        ...
        public FormMain()
        {
            ...
            textBoxCSVFile.Text = @"Statistics.db";
        ...
        private async void buttonCreateCSV_Click(object sender, EventArgs e)
        {
            ...
            await Task.Run(() =>
            {
                var sqlite = new StatisticsSQLite();
                sqlite.Create();
            });
        ...
        private async void buttonExtractInfo_Click(object sender, EventArgs e)
        {
            ...
            await Task.Run(() =>
            {
                ...
                cameras = sqlite.Query("select distinct model from Statistics order by model")
                                .Select(x => x.Model)
                                .Where(x => x != null)
                                .ToList();
                lenses = sqlite.Query("select distinct lenstype from Statistics order by lenstype")
                                .Select(x => x.LensType)
                                .Where(x => x != null)
                                .ToList();
            });
